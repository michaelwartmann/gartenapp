...
# Stage 8.1 — generate the curated Skizzen-Hintergrund WebPs (one-time, committed to repo).
...
16. **AI-augmentierte User-Eingabe statt naivem Trust (Stage 8.2)**: bei Plant-Add suggeriert Gemini live die Kategorie (Live-Hint + Auto-Apply), und beim Submit wird bei zero-overlap zwischen User-Pick und Geminis Vorhersage *replaced statt gemerged* — sonst würden falsche User-Einträge (Kims „Zitrone = Gemüse") sich mit der Korrektur zu unsinnigen Mischungen aufaddieren. Soft-Confirm-Sheet erlaubt User aber das Letztwort. Gleiches Pattern für später denkbar bei Pflanzort, Beet-Eignung, etc. — User-Input ist *Vorschlag*, Gemini-Verdict ist *Sanity-Check*.
17. **Lerntagebuch über Erfolgs-Bilanz (Stage 10)**: die Bilanz-Seite ist nicht Scoring, sie ist Reflexion. Misserfolge (frost, schaedlinge, krankheit, eingegangen) sind genauso wichtig wie kg Tomaten und bekommen eigene „💔 Was nicht klappte"-Sektion mit Lerneffekt-Hints. Der „Pflanze raus"-Knopf zwingt zu einem Reason — kein 1-Tap-Flow ohne Daten. Unhappy-Path-UX ist neutral/hilfreich getönt, niemals zelebrierend oder bestrafend. Default-Reason ist „Saison-Ende" (positiv-grün), Frost ist „event" (neutral-blau), Schädlinge/Krankheit/Eingegangen sind „loss" (terrakotta) — Tone-Klassifikation in `lib/removedReasons.ts` zentral.
